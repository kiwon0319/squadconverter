...
for key in data:
    ...
    id = data [key] ["gun_id"]
    lv = data [key] ["gun_level"]

    stat = CalcStat.CalcStat(int(id), int(lv)).calc()

    data [key] ["life"] = stat ["hp"] * int (data[key] ["number"])
    data [key] ["pow"] = stat ["pow"]
    data [key] ["hit"] = stat ["hit"]
    data [key] ["dodge"] = stat ["dodge"]
    data [key] ["rate"] = stat ["rate"]
